...
def simulate_games(n=50000, output_file="results_unbiased.json"):
    game = TinPattiGame()
    results = []

    for _ in range(n):
        # unbiased → both totals same
        res = game.play(100, 100)

        results.append({
            "A": res["A"],
            "B": res["B"],
            "W": "Tie" if res["WINNER"] == "TIE" else res["WINNER"]
        })

    with open(output_file, "w") as f:
        json.dump(results, f)

    print(f"Saved {n} games to {output_file}")


if __name__ == "__main__":
    import json
    simulate_games(50000)